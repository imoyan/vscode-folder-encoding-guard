...
export class EncodingInventory implements vscode.Disposable {
  ...
  private render(): void {
    ...
    const navigation = `${button("previous", `前の${SCAN_PAGE_SIZE}件`, this.offset === 0)}${button("next", `次の${SCAN_PAGE_SIZE}件`, this.offset + SCAN_PAGE_SIZE >= files.length)}${button("continue", `続きを調べる（最大${SCAN_PAGE_SIZE}件）`, !this.snapshot?.hasMore || this.stale)}`;
...
${this.folder ? "<p>フォルダーの変換は表示中のページに限定せず、配下から候補を確認して選びます。ファイルを開く必要はありません。</p>" : ""}
${navigation}
