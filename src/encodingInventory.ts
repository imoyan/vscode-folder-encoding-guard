...
export class EncodingInventory implements vscode.Disposable {
  ...
  public update(snapshot: EncodingScanSnapshot): void {
    ...
    if ((snapshot.files?.length ?? 0) > previousCount) this.offset = Math.floor(previousCount / SCAN_PAGE_SIZE) * SCAN_PAGE_SIZE;
  ...
  public invalidate(changed: boolean): void {
    ...
    else { this.snapshot = undefined; this.offset = 0; this.stale = false; }
    this.refresh();
  }
  public refresh(): void {
...
</style></head><body><h1>フォルダーの文字コード一覧</h1><p>${esc(this.snapshot?.scopeLabel ?? "フォルダーを選んで調べる")}</p>
${button("choose", "フォルダーを選んで調べる")}${button("refresh", "同じ範囲を調べ直す", !this.snapshot)}${button("continue", `続きを調べる（最大${SCAN_PAGE_SIZE}件）`, !this.snapshot?.hasMore || this.stale)}
${this.snapshot ? "" : "<p>ルール設定なしでもフォルダーを調べられます。</p>"}
<p>保存済みファイルを調べた結果と、エディターの読み込み設定を分けて表示します。ASCII互換・判定不明などは文字コードを一意に特定できません。</p>
${this.conversionActive ? '<p>変換・復元の完了後に一覧を更新します。</p>' : ""}
${this.stale ? '<p class="notice">変更あり・再確認が必要です。前回の結果を残しています。「同じ範囲を調べ直す」で現在の状態を確認してください。</p>' : ""}
<p>${files.length ? this.offset + 1 : 0}–${Math.min(files.length, this.offset + SCAN_PAGE_SIZE)} / 確認済み ${files.length} 件${this.snapshot?.hasMore ? " · 続きあり（全体の件数は未確定）" : ""}</p>
<table><thead><tr><th>ファイル</th><th>保存済みの文字コード<br>（内容からの判定）</th><th>改行</th><th>エディターの読み込み</th><th>この拡張での操作</th><th>操作</th></tr></thead><tbody>${rows || `<tr><td colspan="6">${esc(emptyMessage)}</td></tr>`}</tbody></table>
${button("previous", `前の${SCAN_PAGE_SIZE}件`, this.offset === 0)}${button("next", `次の${SCAN_PAGE_SIZE}件`, this.offset + SCAN_PAGE_SIZE >= files.length)}
${skipped ? `<details><summary>未確認の項目と理由</summary><ul>${skipped}</ul></details>` : ""}
...
<script nonce="${nonce}">const api=acquireVsCodeApi();document.addEventListener('click',event=>{const button=event.target.closest('button[data-action]');if(!button||button.disabled)return;document.querySelectorAll('button').forEach(item=>item.disabled=true);api.postMessage({action:button.dataset.action,index:Number(button.dataset.index),revision:${revision}});});</script></body></html>`;
  ...
  private async handle(message: unknown): Promise<void> {
    ...
    try {
      ...
      else if (action === "previous") this.offset = Math.max(0, this.offset - SCAN_PAGE_SIZE);
      else if (action === "next") this.offset += SCAN_PAGE_SIZE;
