...
ローカルファイルのワークスペース向けに、フォルダー単位の文字コードと改行コードを確認する VS Code 拡張機能です。仮想ワークスペースでは、安全な上限付き読み込みを保証できないため動作対象外です。

プロジェクト内のフォルダーごとに期待する文字コードを登録し、文字コード・改行の不一致や変更を見つけやすくします。選択したファイルの文字コード・改行コードをまとめて変換することもできます。

旧表示名は **Folder Encoding Guard** です。拡張ID（`yusuke-local.folder-encoding-guard`）と設定キー（`folderEncodingGuard.*`）は同じなので、既存の設定を引き続き使えます。

## 使い方

まず **文字コード・改行の状況** の更新ボタンを押すと確認できます。文字コードルールがないワークスペースフォルダーでは、除外設定と件数・サイズ上限の範囲でファイルの文字コードと改行を調べ、改行混在を表示します。文字コードを特定できない場合は `判定不明` として集計・注意表示し、UTF-8を期待値として扱ったり、不一致として警告したりしません。

期待する文字コードとの比較を使う場合は、次の手順でルールを登録します。ルールがあるワークスペースフォルダーでは、そのルールに一致するファイルだけをスキャンします。Git規則・履歴や過去の基準との比較もルール対象で行います。

1. エクスプローラーで対象フォルダーを右クリックします。
2. **文字コード・改行チェック: このフォルダーの文字コードを設定** を選びます。
3. `UTF-8`、`Shift JIS / CP932` などから文字コードを選びます。
...
新しいフォルダーのルールは先頭に追加されるため優先されます。同じパターンの再設定では位置を維持します。ルール一覧の右クリックから文字コードの変更と優先順位の上げ下げができ、番号の小さいルールが優先されます。選択中に設定が変わった場合は古い一覧から上書きしません。
...
- 保存時: 文字コードとルールが異なる場合に通知。通知は保存を停止せず、回答も待ちません。保存後の書き直しは `enforceOnSave` を有効にした場合だけ行います
...
許容した対象は同じツリーの `混在許容` に一覧表示され、右クリックから解除できます。意図した文字コード変更は、検出結果の右クリックから `文字コード変更を確認済みにする` を選ぶと、ローカルの文字コード比較基準だけを更新できます。ファイルは書き換えず、ルール不一致・改行の変更・Git HEADとの差は残します。表示後にファイルや基準が変わった場合、または未保存の場合は受け付けず、再確認を促します。

既定では注意表示のみで、自動の開き直し・保存後の書き直しは行いません。必要な場合だけ `autoReopen` / `enforceOnSave` を明示的に有効にできます。以前から設定で `true` を指定している場合は維持されるため、注意だけにするには `false` にしてください。未保存の内容の開き直しや、表現できない文字を含む保存修復は行いません。
...
| `src/editorEncoding.ts` | 文書の診断、ステータスバー、開き直し、保存修復 |
