...
今回の対象は **確認範囲** に表示します。限定解析は未選択のファイルの比較基準を消しません。ファイルを直接指定した場合は全体の列挙を行わず、そのファイルだけを調べます。フォルダーの列挙は除外設定と件数上限に従います。対象が多すぎる場合は小さいフォルダーに分けてください。直接指定したファイルは除外globより選択を優先しますが、サイズ制限やバイナリ・未保存の除外は維持します。
...
最初に **範囲を選んで解析** で小さい範囲を確認し、続けるときは **範囲を追加して解析**（＋）を使います。確認済みの結果を残し、重複する確認済みファイルは再解析しません。未保存などで未確認だったファイルは、再び範囲に含めると再試行します。キャンセル時は前の結果と範囲を残します。

**更新** は追加済みの範囲を確認し直します。ファイルや設定の変更で結果が古くなった場合は結果を取り下げます。Git比較基準の変更も追加解析時に確認します。結果の保持は現在のウィンドウ内だけで、再起動をまたぐ進捗保存や自動巡回は行いません。1回の検索・解析の件数上限は引き続き適用します。
